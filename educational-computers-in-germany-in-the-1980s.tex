...
LOAD \#10 (2024) and LOAD \#11 (2025) --- the magazine of the VzEkC e.V.
(original PDFs linked under References). English translation and
combined edition by \textbf{Claude (Anthropic, Opus 4.8)}.}
...
Microtronic. So what defines an educational and experimentation
...
\item
  ...
  Microtronic was advertised in 1981 at DM 379, falling to DM 299 by
  1984. For comparison: a VC-20 still cost DM 798 at the end of 1981 ---
  a full week\textquotesingle s salary, given an average German annual
  income of DM 30,900.
\item
...
\pandocbounded{\includegraphics[keepaspectratio,alt={The Busch Microtronic --- here shown with the modern PicoRAM 2090 SD-card expansion.}]{images/microtronic-picoram.jpg}}
...
instructions (random numbers, multiplication, division). The thesis also
reveals that originally a TMS1xxx development system with EPROMs was
used (possibly an HE-2), and that the finished operating system was
handed over to TI "on a floppy disk". As program memory, the developers
connected a 2114 SRAM chip via the TMS1600 "GPIO port" --- although the
TMS family, as a "computer on a chip", provides for no external memory
at all. The 1 KByte of 4-bit words of the 2114 is used as program
memory; the SRAM on the TMS1600 holds the 32 4-bit Microtronic registers
...
Microtronic.) None of this harms the overall impression of a universal
computer; the manual programs are throughout quite general --- the NIM
game actually computes the optimal winning strategy. Interpreting the
instruction set costs time, though: a typical maximum is about 114
instructions per second, dropping to about 40 with the display on; some
programs (e.g. dividing 9999 by 1) take up to 8 seconds.

\textbf{Manual.} The two-volume manual, written by Jörg Vallen and
lovingly illustrated with the "Buschi" mascot, is didactically
...
control, a frequency counter and a reaction-time meter. A clever
...
a "2095" SD-card emulator (building on Martin Sauter\textquotesingle s
2017 protocol decode), and especially \textbf{PicoRAM 2090}, a Raspberry
Pico-based RAM emulator with SD card that replaces the 2114, adds
bank-switched memory expansion and an extensive I/O expansion (speech,
sound, OLED text/graphics, battery-backed clock). It is addressed
through 64 "redundant", ineffective instructions (such as
...
a C/Linux emulator by Ingo D. Rullhusen; a Macintosh app; and
Arduino-based hardware emulators from 2016 onward, including the
"Microtronic 2nd / Next Generation" re-editions built into a Busch
"2070" console (Hackaday "Reinvented Retro Contest" winner, 2021).

\pandocbounded{\includegraphics[keepaspectratio,alt={The Busch Microtronic with the original 2095 cassette interface and a DIY speech synthesizer.}]{images/microtronic-2095.jpg}}

\textbf{Recent developments (2025--2026).} The author has also used the
Microtronic as the brain of larger systems: a recursive Towers of Hanoi
solver drives a physical pan/tilt \textbf{Hanoi robot} and, most
recently, animates the solution on a \textbf{64×32 RGB LED matrix} ---
via a microcontroller that reads the Microtronic\textquotesingle s moves
over a simple 4-bit GPIO protocol (see the \emph{towers-of-hanoi}
repository; a video of the recursive solver running on the Microtronic
is at youtu.be/SwUh-Cs\_eZE). And, most significantly of all, the
original Microtronic firmware ROM was finally recovered and brought back
to life on new hardware: the \textbf{Microtronic Phoenix}, described in
the appendix.
...
come from \textbf{Björn Rathje} (github.com/rab-berlin). Working purely
in the Microtronic\textquotesingle s tiny 256-instruction machine
language, he has produced a remarkable body of work that pushes the
little 4-bit machine far beyond what its 1981 manuals imagined:

\begin{itemize}
\tightlist
\item
  \textbf{Monarch2090} --- a faithful simulation of the legendary
  \emph{Rotomat Monarch} (1972) slot machine on the Microtronic; the
  standout piece, and the most elaborate.
\item
  \textbf{Kniffel2090} --- Yahtzee (Kniffel) for the Microtronic.
\item
  \textbf{Mensch2090} --- "Mensch ärgere dich nicht" (the German Ludo /
  "Sorry!"-style board game).
\item
  \textbf{Invaders2090} --- Space Invaders on the Microtronic.
\item
  \textbf{Life2090} --- Conway\textquotesingle s Game of Life.
\item
  \textbf{BerlinUhr2090} --- the Berlin "set-theory clock"
  (Mengenlehre-Uhr) rendered on the Microtronic.
\item
  \textbf{Film2090} --- "the Microtronic goes to Hollywood": a playful
  animation/film project.
\item
  \textbf{ESP2090} --- a modern take pairing the Microtronic world with
  an ESP32 and MicroPython.
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={The Kosmos CP1, with its spiral-bound manual and the green quick-reference card.}]{images/cp1.jpg}}
...
at 6 MHz. Notably, the foreword of the manual was written by Prof. Karl
Steinbuch, a prominent information-processing professor at the
University of Karlsruhe, though his influence on the design is not
...
manual is lovingly and didactically high-quality, using the "Computron"
character to carry out the CPU operations; its programs resemble the
Microtronic\textquotesingle s (NIM, moon landing, dice, Senso, number
guessing) and kit-based projects (timers, reaction testers, blinking
lights, tone generators, alarms, model-railway control). Compared with
the Microtronic, some programs are less general --- e.g. the
CP1\textquotesingle s NIM is hard-coded for 15 sticks / 3 removable and
uses external buttons for input, since the keyboard cannot be read by
programs. Both software and hardware (Arduino-based) emulations and
re-implementations exist; since the firmware is available and the parts
are standard, faithful re-implementations are easy (e.g. "MiniPC"
emulates the 8049 and runs the original firmware), and an Arduino-based
CP2 cassette emulator has been developed.
...
the author built a small \textbf{CP1 development toolchain}: a real
assembler (mnemonics, labels), and --- after digitizing a genuine CP2
cassette save and reverse-engineering the previously undocumented
CP1/CP2 FSK tape format --- a tool that turns a program into a
\textbf{cassette WAV} you simply play into the CP2, so programs load
without any hand-keying (see the \emph{kosmos-cp1-devel-toolchain}
repository; a video of the recursive Towers of Hanoi running on the CP1
is at youtu.be/SXnRAB-B1f0). This builds on a lively CP1 community ---
the \textbf{asig/kosmos-cp1} emulator (with an integrated assembler and
an SD-card tape emulator), the
\textbf{RalphBln/kosmos-cp1-arduino-cassette-emulator}, and the
\textbf{moosy/kosmos-cp1-toolchain} are all valuable companions.
...
\pandocbounded{\includegraphics[keepaspectratio,alt={The Philips MasterLab and its "Microcomputer Master Lab" manual.}]{images/masterlab.jpg}}
...
author of the manual, whose initials "E.M." also appear in the firmware
EPROM ("COPYRIGHT 1982,1983 (C) GAMA, E.M., \ldots"). What seems certain
is that the MasterLab\textquotesingle s birthplace lies in Hamburg.
...
instructions/second}, making it almost 1300× the Microtronic and almost
80× the CP1. On power-up the monitor shows a friendly "HALLO" --- making
clear the display is good for more than hex; in fact every segment of
every digit can be addressed individually (as the scrolling-text demo
shows).
...
but a HALT can be inserted manually. The manual is a deep, comprehensive
introduction to INS8070 programming --- the CPU even offers 16-bit
operations (the multiply combines A and E into a 16-bit register
multiplied by the 16-bit T register, giving a 32-bit A,E,T result; a
divide exists too), a stack pointer, two pointer registers (SP1, SP2)
for indexed addressing, interrupts, and a wealth of addressing modes
(immediate, absolute, implied, direct, indirect, indexed, relative).
Firmware CALL routines ease programming (display control, 2-/4-digit hex
input, decimal/hex conversion). The biggest shortcoming is the limited
experimentation capability --- the manual only demonstrates a
loudspeaker, a lamp and a simple photoresistor light barrier; more GPIOs
would have helped. The direct, firmware-free coupling of the GPIOs to
the CPU is nonetheless brilliant. The learning material is far more
complex, comprehensive and technical than the others\textquotesingle{}
--- a steeper curve, clearly aimed at adults / teacher training, and
presented bone-dry --- but the MasterLab is the only one offering a
complete, realistic, practical path to microprocessor programming.

\pandocbounded{\includegraphics[keepaspectratio,alt={The Philips MasterLab with an attached experiment box.}]{images/masterlab-expbox.jpg}}

\textbf{Recent developments (2025--2026).} Two projects have since
appeared. Thorsten Brehm ("MacFly") built a complete MasterLab
\textbf{emulator} in JavaScript/HTML-CSS (November 2024), playable
online (github.com/ThorstenBr/MasterLab-MC6400) --- the emulator used as
a reference while developing the vector display below. And the author
built a \textbf{vector-graphics display} for the MasterLab: a program
drives an X-Y oscilloscope to show a rotating 3-D wireframe (cube,
torus, sphere) via a homemade R-2R DAC on the expansion bus --- turning
the INS8070\textquotesingle s speed and the expansion bus into a small
vector-graphics engine (see the \emph{philips-mc6400-vector-graphics}
repository).
...
\toprule\noalign{}
Computer (CPU) & Clock & Execution model & Instructions/sec &
Relative \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
28× \\
Philips MasterLab (INS8070, 8-bit) & 4 MHz & native machine code & ≈
147,050 & ≈ 1,290× \\
\end{longtable}
}
...
\tightlist
\item
  Microtronic --- "The MIPS Monster":
  \url{https://www.youtube.com/watch?v=e8KJ-cnX9bU}
\item
  Kosmos CP1 --- "Another 1983 MIPS Monster":
  \url{https://www.youtube.com/watch?v=5lR29-H8SQQ}
\item
  Philips MasterLab: \url{https://youtu.be/T0yymKe42YQ}
\end{itemize}
...
\textbf{Original German articles (open-access PDFs).} "Der große
Vergleichstest der Lerncomputer -- Teil 1", \emph{LOAD} \#10 (2024), pp.
72--75 ---
\url{https://www.classic-computing.de/wp-content/uploads/2024/10/load10web.pdf}
· continued in \emph{LOAD} \#11 (2025), pp. 72--81 ---
...
\tightlist
\item
  Jörg Vallen, diploma thesis (preamble) ---
  github.com/lambdamikel/Busch-2090
  (\texttt{manuals/joerg-vallen-diplom.pdf})
\item
  \emph{Spielmittel} 4 (1984), 16, pp. 58--60; \emph{mc --- Die
  Mikrocomputer-Zeitschrift}, Nov/Dec 1981, p. 78
\item
  Kosmos CP1 manual --- archive.org/details/cp-1-manual · CP1 info ---
  8bit-homecomputermuseum.at
\item
  MasterLab info --- norbert.old.no/kits/6400/6400.html · MasterLab
  emulator (Thorsten Brehm) --- github.com/ThorstenBr/MasterLab-MC6400
\item
  "Towers of Hanoi" on the CP1 and Microtronic ---
  github.com/lambdamikel/towers-of-hanoi
\item
  CP1 development toolchain (assembler + reverse-engineered cassette
  codec) --- github.com/lambdamikel/kosmos-cp1-devel-toolchain
\item
  MasterLab vector-graphics display (X-Y oscilloscope) ---
  github.com/lambdamikel/philips-mc6400-vector-graphics
\item
  Microtronic Phoenix --- github.com/lambdamikel/microtronic-phoenix
\item
  Emulators: MiniPC (G. Heinrichs); cp1-sim (Java);
  asig/kosmos\_tape\_emulator (CP2); Microtronic emulators by I.
  Rullhusen and others
\item
  Björn Rathje\textquotesingle s Microtronic projects ---
  github.com/rab-berlin (Monarch2090, Kniffel2090, Mensch2090,
  Invaders2090, Life2090, BerlinUhr2090, Film2090, ESP2090)
\item
  Speed benchmarks (GPIO-toggle, instructions/sec): Microtronic ---
  youtube.com/watch?v=e8KJ-cnX9bU · Kosmos CP1 ---
  youtube.com/watch?v=5lR29-H8SQQ · Philips MasterLab ---
  youtu.be/T0yymKe42YQ
\item
  Towers of Hanoi (recursive) running on the machines: Microtronic ---
  youtu.be/SwUh-Cs\_eZE · Kosmos CP1 --- youtu.be/SXnRAB-B1f0
\item
  Author\textquotesingle s YouTube channel ---
  youtube.com/playlist?list=PLvdXKcHrGqhe\_Snxh4nh8RMDz2SiUDCHH
\end{itemize}
...
scene as \emph{LambdaMikel} and \emph{MicrotronicHamburg}.
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A new build running the original firmware: the Microtronic Phoenix.}]{images/phoenix.jpg}}
...
This has now changed through our team\textquotesingle s work. In early
2024, on the occasion of a YouTube video about the "Radio Shack Science
Fair Microcomputer Trainer" (SFMT), the author was contacted by Jason T.
Jacques and "Decle", who had successfully read out the ROM of that very
SFMT. The SFMT uses the "little brother" of the TMS1600 --- the TMS1100.
Might it therefore be possible to read the Microtronic firmware with the
same method? It was not quite that simple. But over the following months
our team developed a procedure that ultimately yielded the read-out
Microtronic ROM. For this, the TMS1600 --- like the TMS1100 before it
--- had to be put into its so-called test mode. The details of the
TMS1600 test mode were not known, and there was no documented case on
the internet; our experiments first had to determine which TMS1600 I/O
pins activate this mode on reset. We also found that the protocol for
serially (bit-by-bit) reading the ROM differs markedly from the
documented TMS1100 protocol. This Microtronic-ROM "archaeology" thus
required several weeks of intensive work and extensive experimentation.
Despite all efforts, a few bits in the ROM image remained ambiguous,
because the read-out process was not 100\% deterministic; these were
corrected manually by Jason through extensive firmware analysis and
"sharp thinking", using "Decle"\textquotesingle s TMS1600 disassembler
(he had also previously built the Arduino-based hardware and software
for reading the SFMT\textquotesingle s TMS1xxx firmware).
...
\textbf{Microtronic Phoenix}. It has been flying again since February
2025 and is made available to the public for replication. It is an
ATmega-based hardware emulator that runs the original Microtronic ROM by
means of a TMS1600 emulator.
...
--- but the 2095 SD-card emulator can be used). More practical is to use
PGM 1/ PGM 2 in Neo mode to access the EEPROM, loading/saving a full
memory image via "slot" numbers 0--41 as "program names" (back in the
day one noted the cassette counter reading instead). But how can the
Phoenix firmware access the EEPROM, given the original ROM has no such
code? A trick: the active firmware can be switched at any time by
pressing reset, and both modes always see the same memory contents ---
so a program can be loaded from EEPROM under Neo and then switched to
Phoenix mode, and run in either. This is handy for development too:
develop in Phoenix mode and switch to Neo for speed, or develop in Neo
(more direct EEPROM access) and switch to Phoenix for the most authentic
behavior. A further advantage of Neo is that the virtual machine
language (or the redundant instructions) can easily be adapted for
one\textquotesingle s own experiments --- it is, after all, a simple
Arduino "C" program (not so easy with the original firmware). Thus a
Microtronic variant was developed that acts as a MIDI drum computer /
"drum sequencer", requiring extensive instruction-set extensions and
machine-language changes; this "Microtronic drum computer" won a prize
in the RetroChallenge RC 2021/10.
